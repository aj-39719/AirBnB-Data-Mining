...
rmse(test_set_location$review_scores_location,predict(lm_reg_onlyl,test_set_location[,c("latitude","longitude"),drop=FALSE]))
#Test RMSE = 0.4326636
par(mfrow=c(2,2))
...
pred_knn = predict(knn_reg, newdata = test_set_location)
RMSE(pred_knn,test_set_location$review_scores_location)
# Test RMSE =0.4201638  
...
# ***** Decision Tree Model ******
colnames(training_set_communication) <- make.names(colnames(training_set_communication))
colnames(test_set_communication) <- make.names(colnames(test_set_communication))
...
# Cross-validation to choose the best tree
training_set_communication1 = training_set_communication %>% mutate_if(sapply(training_set_communication, is.factor), as.numeric)
test_set_communication1 = test_set_communication %>% mutate_if(sapply(test_set_communication, is.factor), as.numeric)
colnames(training_set_communication1) <- make.names(colnames(training_set_communication1))
colnames(test_set_communication1) <- make.names(colnames(test_set_communication1))


cp.grid = expand.grid(.cp = (0:10)*0.001)
tree_reg = train(review_scores_communication ~ . ,
                 data = training_set_communication1, method = 'rpart',
                 trControl = trainControl(method = 'repeatedcv' , number = 10, repeats = 3),
                 tuneGrid = cp.grid )
tree_reg 
best.tree = tree_reg$finalModel
prp(best.tree) # the best cp is 0.004               
rmse (training_set_communication1$review_scores_communication,predict (best.tree , newdata =training_set_communication1))
rmse (test_set_communication1$review_scores_communication,predict (best.tree , newdata =test_set_communication1)) 
...
training_set_checkin1 = training_set_checkin %>% mutate_if(sapply(training_set_checkin, is.factor), as.numeric)
test_set_checkin1 = test_set_checkin %>% mutate_if(sapply(test_set_checkin, is.factor), as.numeric)
lm_reg = train( review_scores_checkin~.,
                data =training_set_checkin1 ,
                method = 'lm',
                trControl = trainControl(method = 'repeatedcv' , number = 10, repeats = 3),
                preProcess = c('center','scale'))
lm_reg$finalModel
...
## Decision tree regression
tree_checkin<- tree (review_scores_checkin~. , data=training_set_checkin1)
summary(tree_checkin)
plot (tree_checkin)
text (tree_checkin , pretty = 0)
# rmse in training set =0.4729908; rmse in testing set is 0.4584032
mean ((predict (tree_checkin , newdata =training_set_checkin1) - training_set_checkin1$review_scores_checkin)^2) 
mean ((predict (tree_checkin , newdata =test_set_checkin1) - test_set_checkin1$review_scores_checkin)^2)
...
tree_reg = train(review_scores_checkin~.,
                 data = training_set_checkin1, method = 'rpart',
                 trControl = trainControl(method = 'repeatedcv' , number = 10, repeats = 3),
                 tuneGrid = cp.grid )
tree_reg 
best.tree = tree_reg$finalModel
prp(best.tree) # the best cp is 0.001  
# rmse for training set is   0.4686987;
mean ((predict (best.tree , newdata =training_set_checkin1) - training_set_checkin1$review_scores_checkin)^2) 
# rmse for training set is  0.4537099
mean ((predict (best.tree , newdata =test_set_checkin1) - test_set_checkin1$review_scores_checkin)^2) 
...
# eps-regression: no control of number of support vectors
svmfit1 = svm(review_scores_checkin~. ,data=training_set_checkin1, kernel="radial" ,scale=TRUE) # needs around 5 minutes to run
summary(svmfit1) 
# training set rmse = 0.4943948
rmse(training_set_checkin1$review_scores_checkin,predict(svmfit1,training_set_checkin1)) 
# Testing set rmse =0.4774624
rmse(test_set_checkin1$review_scores_checkin,predict(svmfit1,test_set_checkin1))

# fit in linear kernal svm regression
svmfit2 = svm(review_scores_checkin~. ,data=training_set_checkin1, kernel="linear" ,scale=TRUE)
# training set rmse =  0.5010276
rmse(training_set_checkin1$review_scores_checkin,predict(svmfit2,training_set_checkin1))
# testing set rmse = 0.4848455
rmse(test_set_checkin1$review_scores_checkin,predict(svmfit2,test_set_checkin1))
```
```{r}
# rating
training_set_rating = training_set[,-c("host_location","host_neighbourhood","neighbourhood_cleansed","property_type","room_type","review_scores_accuracy","review_scores_communication","review_scores_location","review_scores_value","review_scores_checkin","review_scores_cleanliness")]

test_set_rating = test_set[,-c("host_location","host_neighbourhood","neighbourhood_cleansed","property_type","room_type","review_scores_accuracy","review_scores_communication","review_scores_location","review_scores_value","review_scores_checkin","review_scores_cleanliness")]

hist(training_set_rating$review_scores_rating) # not normal distributed log doesn't work
summary(training_set_rating )
training_set_rating1 = training_set_rating %>% mutate_if(sapply(training_set_rating, is.factor), as.numeric)
test_set_rating1 = test_set_rating %>% mutate_if(sapply(test_set_rating, is.factor), as.numeric)

# linear regression
lm_full = lm(review_scores_rating~.,data =training_set_rating1)
summary(lm_full)
# can explain 10% variance
par(mfrow=c(2,2))
plot(lm_full)# heavy tails residuals are not normally distributed
# training rmse = 0.330371; testing rmse = 0.3203817
rmse(training_set_rating1$review_scores_rating,predict(lm_full,training_set_rating1))
rmse(test_set_rating1$review_scores_rating,predict(lm_full,test_set_rating1))
# rank-deficient caused by the colinearity in the amenities dummy variables.
# we extract distinct charactors and count them. \


#log lm reg[not selected]
lm_log = lm(log(review_scores_rating+1)~.,data =training_set_rating1)
...
#decision tree
par(mfrow=c(1,1))
colnames(training_set_rating) <- make.names(colnames(training_set_rating))
colnames(test_set_rating) <- make.names(colnames(test_set_rating))
colnames(training_set_rating1) <- make.names(colnames(training_set_rating1))
colnames(test_set_rating1) <- make.names(colnames(test_set_rating1))
...
cp.grid = expand.grid(.cp = (0:10)*0.001)
tree_reg = train(review_scores_rating~.,
                 data = training_set_rating1, method = 'rpart',
                 trControl = trainControl(method = 'repeatedcv' , number = 10, repeats = 3),
                 tuneGrid = cp.grid )
tree_reg 
best.tree = tree_reg$finalModel
prp(best.tree) # the best cp is  0.005  
# rmse for training set is  0.4982808;
rmse (training_set_rating1$review_scores_rating,predict (best.tree , newdata =training_set_rating1)) 
# rmse for test set is  0.4823962
rmse (test_set_rating1$review_scores_rating, predict (best.tree , newdata =test_set_rating1))
